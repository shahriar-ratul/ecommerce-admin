...
export default function VerificationTable() {
  ...
  const onConfirm = async () => {
    ...
    if (actionType === "changeStatus") {
      const { data } = await axios.put(
        `/api/v1/users-verify/verify/${selectedRow?.id}`
      );

      if (data) {
        toast.success(`${selectedRow?.username} User verified successfully`, {
          style: {
...

    {
      ...
      cell: ({ row }) => (
        <>
          <Button
            variant="default"
            onClick={() => router.push(`/user/verification/${row.original.id}`)}
          >
  ...
  const fetchData = async (page: number, limit: number, search: string) => {
    ...
    const { data } = await axios.get(
      `/api/v1/users-verify?page=${page}&limit=${limit}&search=${search}`
    );
