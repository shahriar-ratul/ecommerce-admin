...
export const UpdateProductForm = ({ item }: PropData) => {
  ...
  const defaultValues = {
    ...
    batch: "",
    productId: "",
    category: item.category ? String(item.category.id) : "",
    source: item.source ? String(item.source.id) : "",
    qty: "",
    costPerUnit: "",
    localShippingCost: "",
    ...
    partnerPrice: "",
    platform: "",
    route: item.route ? String(item.route.id) : "",
    trackingStatus: item.trackingStatus ? String(item.trackingStatus) : "",
    trackingNumberLocal: "",
  ...
  useEffect(() => {
    if (item) {
      form.setValue("productId", item.stockID);

      if (item.entryDate) {
        form.setValue("entryDate", new Date(item.entryDate));
      }

      form.setValue("batch", item.batch);
      form.setValue("name", item.name);
      form.setValue("qty", String(item.qty));
      form.setValue("costPerUnit", String(item.costPerUnit));
      form.setValue("localShippingCost", String(item.localShippingCost));
      form.setValue(
        "internationalShippingCost",
        String(item.internationalShippingCost)
      );
      form.setValue("customsTax", String(item.customsTax));
      form.setValue("salePrice", String(item.salePrice));
      form.setValue("marketPrice", String(item.marketPrice));
      form.setValue("partnerPrice", String(item.partnerPrice));

      if (item.category) {
        form.setValue("category", String(item.category.id));
      }

      if (item.source) {
        form.setValue("source", String(item.source.id));
      }

      if (item.platforms) {
        const platforms = item.platforms.map((item: any) => {
          return {
      ...
      if (item.route) {
        form.setValue("route", String(item.route.id));
      }

      if (item.trackingStatus) {
        form.setValue("trackingStatus", String(item.trackingStatus));
      }

      if (item.trackingNumberLocal) {
        form.setValue("trackingNumberLocal", item.trackingNumberLocal);
      }

      if (item.trackingNumberInternational) {
        form.setValue(
          "trackingNumberInternational",
          item.trackingNumberInternational
        );
      }
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      await axios
        .put(`${BackendUrl}/api/v1/stocks/${item.id}`, formData, {
          headers: {
        ...
        .then(res => {
          const message =
            (res.data.data.message as string) || "updated successfully";
